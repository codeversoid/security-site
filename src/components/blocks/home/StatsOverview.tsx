...
export default function StatsOverview() {
  ...
  return (
    <section className="mx-auto max-w-[1200px] px-4 md:px-8 xl:px-12 py-12 lg:py-20">
      <div className="rounded-2xl border bg-card/40 p-6 md:p-8">
        <div className="flex items-end justify-between gap-6 flex-wrap">
          <div>
            <p className="text-sm font-medium tracking-wide text-accent">Sorotan Kinerja</p>
        ...
        <div className="mt-6 grid grid-cols-12 gap-4 md:gap-6">
          {stats.map((s, i) => (
            <motion.div
              ...
              className="col-span-12 sm:col-span-6 lg:col-span-3 rounded-xl border bg-card/30 p-5"
            >
              <div className="flex items-center gap-3">
                <span className="inline-block h-2 w-2 rounded-full bg-accent" />
                <span className="text-3xl font-bold tracking-tight text-foreground">{s.value.toLocaleString()}</span>
              </div>
